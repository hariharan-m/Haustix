...
int i2c_read_1_byte(uint8_t addr, uint8_t *result)
{
    ...
    //while (ISR != 0x45) // wait for transmit empty interrupt  IDBR transmit empty (1),  Unit Busy (1), R/nW bit (1)
   // { //01000101
       // mdelay(5);
   // }
    printk(KERN_INFO "IDBR: %04x\n", IDBR);
    ...
    // Step 6  IDBR receive full (1), Unit Busy (x), R/nW bit (1), ACK/NAK bit (1)
    printk(KERN_INFO "===%s: step 6\n", __FUNCTION__);
   // while (ISR != 0x87) // wait for receive full interrupt //10000111
   //   {  //mdelay(5);
//}
    printk(KERN_INFO "myi2c: ISR %04x\n", ISR);
